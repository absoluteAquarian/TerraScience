...
//Copy of MS.Internal.PriorityQueue:  https://referencesource.microsoft.com/#PresentationCore/Shared/MS/Internal/PriorityQueue.cs
//Modified slightly to better suit the mod's usage
namespace TerraScience.Systems.Pathfinding{
	...
	internal class PriorityQueue<T>
	{
		...
		internal void UpdateElement(T element){
			//If the element doesn't exist in the heap, just add it via Push
			//Otherwise, replace its entry and force the queue to re-heap itself
			int index = Array.IndexOf(_heap, element);
		...
		internal int FindIndex(T element)
			=> Array.IndexOf(_heap, element);

		internal T GetHeapValueAt(int index)
			=> _heap[index];
		...
		private int SiftDown(int index){
			...
			while(leftChild < _count){
				...
				int bestChild = (rightChild < _count && _comparer.Compare(_heap[rightChild], _heap[leftChild]) < 0)
					? rightChild
					: leftChild;
