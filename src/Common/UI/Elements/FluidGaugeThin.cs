...
namespace TerraScience.Common.UI.Elements {
	// A copy of FluidGauge.cs, but thinner
	public class FluidGaugeThin : UIElement, IColorable {
		...
		public FluidGaugeThin(double max, int segmentColumns, int segmentRows) {
			SetMaxCapacity(max);
			SetSegmentDimensions_WithoutRecalculate(segmentColumns, segmentRows);
		}

		public FluidGaugeThin(double max, float pixelWidth, float pixelHeight) {
			SetMaxCapacity(max);
		...
		const int singleSegmentWidth = 4;
		...
		protected override void DrawSelf(SpriteBatch spriteBatch) {
			Texture ??= ModContent.Request<Texture2D>("TerraScience/Assets/UI/tank thin");
